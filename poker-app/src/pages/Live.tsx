...
import type { CardObj, Player, StreetData } from '../lib/types'
...
export function Live({
    ...
    return (
      ...
      >
        <motion.div
          ...
          style={{ fontSize: 72, marginBottom: 20, color: 'var(--gold)', opacity: 0.7, lineHeight: 1 }}
        >
          ♠
        </motion.div>
        <div style={{ fontFamily: 'var(--fs)', fontSize: 22, fontWeight: 800, color: 'var(--t1)', marginBottom: 10 }}>
          No active game
        </div>
        <div style={{ fontSize: 14, color: 'var(--t3)', lineHeight: 1.7, marginBottom: 28 }}>
          Start a session to begin tracking<br />hands, rebuys, and cashouts.
        </div>
        <div style={{ display: 'flex', gap: 6, marginBottom: 12 }}>
          {['♠', '♥', '♦', '♣'].map((s, i) => (
            <motion.span key={s}
              ...
              style={{ fontSize: 24, color: i % 2 === 0 ? 'var(--gold)' : 'var(--violet)', opacity: 0.5 }}
            >{s}</motion.span>
          ))}
        </div>
        <div style={{ fontSize: 12, color: 'var(--t4)', fontStyle: 'italic' }}>
          Tap the + button in the header to start
        </div>
      </motion.div>
    )
  }
  ...
  // Session doc not yet loaded from Firestore
  if (!sess) {
    return (
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontFamily: 'var(--fs)', fontSize: 36, fontWeight: 900, color: 'var(--gold)', lineHeight: 1 }}>
              {(lh.count ?? 0) + 1}
            </div>
            <div style={{ fontSize: 10, color: 'var(--t3)', letterSpacing: '0.1em', textTransform: 'uppercase', marginTop: 2 }}>Hand #</div>
          </div>
        </div>
      </div>
